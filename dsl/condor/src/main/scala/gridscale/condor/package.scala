...
package object condor {
  ...
  object impl {
    ...
    def queryState[M[_]: Monad, S](server: S, job: BatchJob)(hn: HeadNode[S, M], error: ErrorHandler[M]): M[JobState] = {
      ...
      val queryInQueueCommand = shell.BashShell.bashCommand(s"condor_q ${job.jobId} -long -attributes JobStatus")
      // ...but if the job is already completed, his state is returned by condor_history...
      val queryFinishedCommand = shell.BashShell.bashCommand(s"condor_history ${job.jobId} -long")
